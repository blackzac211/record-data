...
public class GraphController {
	...
	public static void main(String[] args) {
		try {
			...
			startDate.set(2020, 6, 1, 0, 0, 0);
			Calendar endDate = Calendar.getInstance();
			endDate.set(2020, 7, 31, 23, 59, 59);
			String startTime;
			String endTime;
			String curTime;
			...
			while (true) {
				...
				// sql = "SELECT metid, metdate, starttime, endtime, usercnt " + "FROM hero2 " + "WHERE metdate = ? "+ "ORDER BY starttime ";
				
				sql = "SELECT rec_id, rec_startdate, rec_starttime, rec_enddate, rec_endtime " +
						"FROM rec_data " +
						"WHERE rec_startdate = ? " + 
				...
				while (rs.next()) {
					...
					// sql_detail = "SELECT SUM(usercnt) FROM rec_data " + "WHERE metdate = ? AND ((starttime >= ? and starttime <= ?) OR (endtime >= ? AND endtime <= ?))";
					sql_detail = "SELECT count(*) " + 
							"FROM rec_data " + 
							"WHERE rec_startdate = ? AND ((rec_starttime >= ? and rec_starttime <= ?) " +
								"OR (rec_endtime >= ? AND rec_endtime <= ?))";
					...
					if (map.get(curDate) < detailCnt) {
						while (true) {
							...
							// sql_detail = "SELECT SUM(usercnt)" + "FROM hero2 " + "WHERE metdate=? AND starttime <= ? AND endtime >= ?";
							sql_detail = "SELECT count(*) " + 
									"FROM rec_data " + 
	...
	public void selectRecordLogs(String id_up, HttpServletRequest request, HttpServletResponse response,
			HttpSession session) {
		try {
